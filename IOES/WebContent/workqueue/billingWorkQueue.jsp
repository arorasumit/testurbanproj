...
<html>
<head>
<title>:: Billing Work Queue: Integrated Order Entry System</title>
...
function fnViewOrder(orderNo,ordertype)
{
    ...
    if(ordertype=="Change")
    {
	    ...
	    //document.forms[0].submit();
	    //Start[003]
	    var path = "<%=request.getContextPath()%>/ChangeOrderAction.do?method=incompleteOrder&draft=1&orderNo="+orderNo+"&modeName="+modeValue;
		window.open(path,'',"scrollbars=yes,resizable=yes,width=1030,height=650,top=0,left=0");
		//End[003]
	}
...
function searchSubmit()
{
	...
	myForm.action="<%=request.getContextPath()%>/universalWorkQueueAction.do?methodName=viewBillingOrderQueue";
	showLayer();
	myForm.submit();
...
function isBlankForm()
{
	...
	if((quoteNumber==null && orderType==null && source==null && crmOrderNo==null && accountNo == null  && accountName == null  && fromDate == null && toDate == null && subChangeType == null
...
<html:form action="/universalWorkQueueAction" method="post" styleId="searchForm">
	...
	<table border="0" align="center" cellpadding="0" cellspacing="0">
		<tr align="left">
			<logic:equal value="1" property="isBillingOrder"
				name="UniversalWorkQueueLogicalFormBean">
				<td class="head">&nbsp;Billing Work Queue</td>
			</logic:equal>
		</tr>
		...
		<tr>
			...
			<table border="1" id="t1" width="100%" style=" border-color:#CCCC99; border: 1px ;font-family:Verdana, Arial, Helvetica, sans-serif; font-size:11px; color:#000000; border:2px #CCCC99 solid;" cellpadding="2">
						...
						<tr>
							<td width="5%" align="center"><b>SNo.</b></td>
										...
										<td width="5%" align="center"><b>	
											<logic:present name="billingWorkQueueList" scope="request">
													<logic:notEmpty name="billingWorkQueueList" scope="request">
														<a href="#" onclick="sortSubmit('ORDERNO')">Order No</a>
												</logic:notEmpty>
												<logic:empty  name="billingWorkQueueList" scope="request">
													Order No
												</logic:empty>
											</logic:present></b></td>
								    ...
								    <td width="5%" align="center"><b>	
									 ...
									 <td width="5%" align="center"><b>	
											<logic:present name="billingWorkQueueList" scope="request">
													<logic:notEmpty name="billingWorkQueueList" scope="request">
														<a href="#" onclick="sortSubmit('SOURCE')">Source Name</a>
												</logic:notEmpty>
												<logic:empty  name="billingWorkQueueList" scope="request">
												Source Name
												</logic:empty>
									 ...
									 <td width="5%" align="center"><b>	
											<logic:present name="billingWorkQueueList" scope="request">
													<logic:notEmpty name="billingWorkQueueList" scope="request">
														<a href="#" onclick="sortSubmit('QUOTENO')">Quote Number</a>
												</logic:notEmpty>
												<logic:empty  name="billingWorkQueueList" scope="request">
												Quote Number
												</logic:empty>
					   	...
					   	<tr class="th">	
							...
							<td width="5%"><html:select property="searchSource"
							...
							<td width="5%"><html:text property="searchQuoteNumber"
								maxlength="8" size="5" styleId="id_orderdate_search"
								onkeydown="if(event.keyCode == 13){validation(this)};"
								 />
							</td>
				...
				<logic:equal value="1" property="isBillingOrder" 
					...
					<logic:notEmpty name="billingWorkQueueList" scope="request">
						<logic:iterate id="row"	name="billingWorkQueueList" 
                        ...
                        <!--[00011] End-->
							...
							<td width="1.5%"><bean:write name="row"
								property="searchAccountName" />&nbsp;</td>
							<td width="9%"><bean:write name="row"
								property="searchfromDate" />&nbsp;</td>
							<td width="6%"><bean:write name="row"
								property='searchSource' />&nbsp;</td>
							<td width="4%"><bean:write name="row"
								property='searchQuoteNumber' />&nbsp;</td>
							<td width="6%"><bean:write name="row"
							...
							<!--[00011] End-->
							<td width="6.5%"><bean:write name="row"
								property='industrySegment' />&nbsp;</td>
							<td width="9.5%"><bean:write name="row"
