...
<!--[002]	 ANIL KUMAR	    11-feb-2011 				Annotation field added in charges section against charges name -->
<!--[003]	 Rohit Verma	17-feb-2011 				Logic Start Date and End Date Label Chnaged -->
...
<!--[007] 	 RAGHU	16-Mar-11	00-05422		TotalCharges auto populate in service summary in Toatal Feild-->
...
<!--[010]	 Saurabh		03-Oct-2011				    Some rows and column added  -->
...
/*function fillChargeNames(var_counter, chargeTypeValue,chargeNameID){
	try{
	    ...
	    for (i = local_RowCount-1; i >0 ; i--){//[001]	End
...
//	var account = parseInt(document.getElementById('hdnPOAccount').value);
	...
	//[013] Start
	if(document.getElementById('hdnHardwareInfo').value==1){
		str = "<input onmouseover='getTip(value)' onmouseout='UnTip()' type='text' style='width:175px;' isRequired='0' Disp='' class='inputBorder1' name='txtCAnnotation' id='txtCAnnotation"+counter+"' maxlength='240'  onblur=\"if(this.value.length > 0){if(CheckMinLength(this)){return ValidateTextField(this,path,'Annotation')}}\" >";
	}else{
		str = "<input onmouseover='getTip(value)' onmouseout='UnTip()' type='text' style='width:175px;' isRequired='0' Disp='' class='inputBorder1' name='txtCAnnotation' id='txtCAnnotation"+counter+"' maxlength='340' onblur=\"if(this.value.length > 0){if(CheckMinLength(this)){return ValidateTextField(this,path,'Annotation')}}\" >";
	}
	//[013] End
	CellContents = str;
	tblcol.innerHTML = CellContents;
	...
	str="<table><tr><td><select  name='txtCStartDate' id='txtCStartDate"+counter+"' isRequired='0' Disp='' class='dropdownMargin' style='width:125px;'><option value='-1'>Select Start Date Logic</option><option value='BTD'>Billing Trigger</option></select></td> ";
	...
	tblcol.align = "left";
	tblcol.vAlign="top";
	str="<input type='text' style='width:100px;' value='0'  isRequired='0' Disp='' maxlength='4' class='inputBorder' name='txtStartDays'  id='txtStartDays"+counter+"' onblur='if(this.value.length > 0){return checknumber(this)}'>";
	CellContents = str;
	tblcol.innerHTML = CellContents;
	...
	tblcol.vAlign="top";
	tblcol.className="innerdata col4";
	str="<table><tr><td><select  name='txtCEndDate' id='txtCEndDate"+counter+"' isRequired='0' Disp='' class='dropdownMargin' onchange='changeEndDays("+counter+");' style='width:125px;'><option value='-1'>Select End Date Logic</option></select></td>";
	...
	str ="<input name='chkexcludeCharges' class='inputBorder' id='chkexcludeCharges"+counter+"' type='checkbox'>";
	CellContents = str;
	tblcol.innerHTML = CellContents;
	...
	//ctrlArry1[11] =  document.getElementById("cmbLineItem"+counter);
	//ctrlArry1[12] =  document.getElementById("cmbSubLineItem"+counter);
...
{	
	try
	{
		...
		//{
			...
			/*var billFormatCombo=document.getElementById("txtBillingformat");
			var index=billFormatCombo.selectedIndex;
			var billFormatType=billFormatCombo.options[index].text;
			alert(billFormatType);*/
			jsonrpc = new JSONRpcClient("<%=request.getContextPath()%>" + "/JSON-RPC");
...
function fillChargeName(var_counter,ChargeType)
{//alert(" fillChargeName 11111111111111111111 : " + var_counter);
	try
	{
			//cache charge type
			
			var chargetype = ChargeType;
		//	alert("chargetype :   @@@@@@@@@@@@@ "+ document.getElementById("ddCType"+var_counter).value );
		//	alert("hdnServiceDetailID : "+ document.getElementById("hdnServiceDetailID").value);
			var productid = document.getElementById("hdnServiceDetailID").value;
			//alert("productid : "+ productid );
			jsonrpc = new JSONRpcClient("<%=request.getContextPath()%>" + "/JSON-RPC");
			chargeCombo = jsonrpc.processes.populateChargeName(chargetype,productid);
			//alert(" end of populateChargeName   " + chargeCombo.list.length);
			gchargeName=new Array();
			gchargeid=new Array()
			for(z=0;z<chargeCombo.list.length;z++)
		    {
		   // alert("Charge Name :  "+chargeCombo.list[z].chargeName);
		    //	    alert("Charge Id :  "+chargeCombo.list[z].chargeId);
		    	gchargeName[z] = chargeCombo.list[z].chargeName;
...
function CheckRCRequiredForCLEP(chargetype,var_counter){
			...
			var order_creation_source = callerWindowObj.document.getElementById('hdnOrderCreationSourceId').value;
	  		if(order_creation_source==ORDER_CREATION_SOURCE && chargetype.value=="1"){	  		    
	  			...
	  			ChargeNameCombo.value=-1;
	  			document.getElementById("chargeName"+var_counter).value = "";
	  			document.getElementById("txtCAnnotation"+var_counter).value = "";
	  			return false;
	  		}
}
...
<!-- Start:Added below code for Charge Table freeze -->
...
<fieldset class="border1" id="chargesInfoFieldsetId">
		...
		<!----------------------------------------- [ PAGING-CHARGES-LINE-04-12-2012 ]------------------------------------------------------------------- -->
				...
				<tr>
					<td align="center"><a href="#" id= "first" onclick="FirstPage('CHARGEINFOID','VIEWCHARGETABLE')">First</a>&nbsp;&nbsp;<a href="#" id="next" onclick="NextPage('CHARGEINFOID','VIEWCHARGETABLE')">Next</a></td>
					<td align="center">
					...
					<td align="center"><a href="#" id="prev" onclick="PrevPage('CHARGEINFOID','VIEWCHARGETABLE')">Prev</a>&nbsp;&nbsp;<a href="#" id="last" onclick="LastPage('CHARGEINFOID','VIEWCHARGETABLE')">Last</a></td>
				</tr>
				</table>
		<!----------------------------------------- [ PAGING-CHARGES-LINE-04-12-2012 ]------------------------------------------------------------------- -->
...
<!--                    </div>-->
			...
			<tr>
				<td colspan="9" align="center">
					    <input type="button" id = "btnAddContact" name="btnAddContact" value="Add Charges" onClick="AddCharge()">
					    <input type="button" id = "btnDeletePoDetail" name="btnDeletePoDetail" value="Delete Charges" onClick="DeleteCharge()">
