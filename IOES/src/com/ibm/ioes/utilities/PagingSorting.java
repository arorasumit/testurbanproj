...
public class PagingSorting 
{
	...
	public PagingSorting(boolean pagingToBeDone,boolean sortingToBeDone,int noOfRecords)
	{
		this.pagingToBeDone=pagingToBeDone;
		this.sortingToBeDone=sortingToBeDone;
		pageRecords=noOfRecords;
	...
	public void sync(String type)
	{
		...
		else // if("jdbc".equals(type))  //for jdbc
		{
			this.startRecordId = 1+(pageNumber-1)*pageRecords;
			this.mode=jdbc;
		}
...
//	TODO Changes Made By Sumit For Paging if page no is 0 than initial page no should be 1
				...
				throws IOESException
	{
		...
		try {
			...
			throw new IOESException("SQL Exception : "
			...
			try {
				...
				logger
						.error(ex.getMessage()
								...
								+ this.getClass().getSimpleName());
				throw new IOESException("Exception : " + ex.getMessage(),
						ex);
			}
		}
	...
	public static String DB_Asc_Desc(String sortByOrder) {
	...
	public static String DB_Asc_Desc1(String sortByOrder) {
		if(sortByOrder == null || "".equals(sortByOrder))
		{
			return "ASC";
		}
	...
	public void setEndRecordIdForNoOfRecordsCount() {
		endRecordId=startRecordId+1;		
	}
}
