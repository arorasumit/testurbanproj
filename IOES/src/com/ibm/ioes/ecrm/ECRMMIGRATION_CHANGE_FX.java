...
public class ECRMMIGRATION_CHANGE_FX {

	
	//public static String orderNo="319582";1029095
	public static String orderNo="1029095";//1261731";//1441854";//"1153911";//"1362935";//"319582";	//"358019";	//"358062";
	...
	private static String strGetOrderHeaderFromCRM2="SELECT IBMOE_ORDER_HEADER.CRM_ORDER_ID,IBMOE_ORDER_HEADER.ORDERTYPE,IBMOE_ORDER_HEADER.ORDERSOURCE,COALESCE(IBMOE_ORDER_HEADER.QUOTENUMBER,'0') AS QUOTENUMBER,IBMOE_ORDER_HEADER.CURRENCY,IBMOE_ORDER_HEADER.M6_ORDER_STATUS,"
		...
		+" (SELECT SERVICE_ORDER_TYPE FROM IBMOE_ORDER_SERVICE_DETAILS WHERE CRM_ORDER_ID=IBMOE_ORDER_HEADER.CRM_ORDER_ID) AS SUBCHANGE_TYPE_ID,"
		+" (SELECT "
		...
		+" FROM IBMOE_ORDER_HEADER IBMOE_ORDER_HEADER"
		+" WHERE IBMOE_ORDER_HEADER.CRM_ORDER_ID IN(SELECT CRM_ORDER_ID FROM IBMOE_ORDER_SERVICE_DETAILS WHERE LOGICAL_SI_NUMBER='"+orderNo+"')";
    ...
    +" LAST_UPDATED_BY FROM IBMOE_ORDER_HEADER_ATTRIBUTES"
    +" WHERE CRM_ORDER_ID IN(SELECT CRM_ORDER_ID FROM IBMOE_ORDER_SERVICE_DETAILS WHERE LOGICAL_SI_NUMBER='"+orderNo+"')";
    ...
    +" EMAIL, CELLTELEPHONENR, CRM_ORDER_ID, FAXNR,  CREATION_DATE, CREATED_BY,"
    +" LAST_UPDATE_DATE, LAST_UPDATED_BY FROM IBMOE_ORDER_CONTACT "
    +" WHERE CRM_ORDER_ID IN(SELECT CRM_ORDER_ID FROM IBMOE_ORDER_SERVICE_DETAILS WHERE LOGICAL_SI_NUMBER='"+orderNo+"')";
	...
	private static String strGetOrderServiceFromCRM2="select distinct SERVICE.SERVICE_NO, SERVICE.M6_PRODUCT_ID,SERVICE_TYPE AS SERVICESTAGENAME,SERVICE.EFFECTIVE_START_DATE," 
			...
			+" (SELECT MAX(SERVICE_NO) FROM IBMOE_ORDER_SERVICE_DETAILS"
			+" WHERE LOGICAL_SI_NUMBER = '"+orderNo+"' AND SERVICE_NO <> SERVICE.SERVICE_NO AND SERVICE_NO < SERVICE.SERVICE_NO) AS OLDSERVICEID,"
			+" CASE WHEN (SELECT MAX(SERVICE_NO) FROM IBMOE_ORDER_SERVICE_DETAILS WHERE LOGICAL_SI_NUMBER = '"+orderNo+"' ) =SERVICE.SERVICE_NO" 
...
*/	
	...
	private static String strGetOrderLineFromCRM2="SELECT * FROM ("
		...
		+" WHERE ol1.ib_order_line_id = "
		+" ("
		+" SELECT MIN(OLC.ib_order_line_id) FROM ibmoe_order_line OLC"
		+" WHERE  OLC.CRM_ORDER_ID IN ("
		+" SELECT distinct IBMOE_ORDER_SERVICE_DETAILS.CRM_ORDER_ID "
		+" FROM IBMOE_ORDER_SERVICE_DETAILS "
		...
		+" FROM ibmoe_order_line ol1"
		+" WHERE ol1.ib_order_line_id = "
		+" (SELECT OLC.ib_order_line_id FROM ibmoe_order_line OLC"
		+" WHERE  OLC.CRM_ORDER_ID IN ("
		+" SELECT distinct IBMOE_ORDER_SERVICE_DETAILS.CRM_ORDER_ID FROM IBMOE_ORDER_SERVICE_DETAILS WHERE LOGICAL_SI_NUMBER='"+orderNo+"' "
		+" and IBMOE_ORDER_SERVICE_DETAILS.CRM_ORDER_ID=IBMOE_ORDER_LINE.CRM_ORDER_ID"
		+" ) AND OLC.CHILD_SERVICE_KEY=(SELECT PARENT_SERVICE_KEY FROM IBMOE_ORDER_LINE  ol WHERE  ol.ORDER_LINE_ID=IBMOE_ORDER_LINE.order_line_id)"
		+" )"
		+" )"
		...
		+" LEFT JOIN IBMOE_BILLING_ACCOUNTS IBMOE_BILLING_ACCOUNTS ON IBMOE_BILLING_ACCOUNTS.CHILD_ID=IBMOE_ORDER_LINE.CHILD_ID "
		+" INNER JOIN IBMOE_ORDER_SERVICE_DETAILS SERVICE ON SERVICE.SERVICE_NO= IBMOE_ORDER_LINE.SERVICE_NO "
		+" WHERE IBMOE_ORDER_LINE.CRM_ORDER_ID IN(SELECT MAX(CRM_ORDER_ID) FROM IBMOE_ORDER_SERVICE_DETAILS WHERE LOGICAL_SI_NUMBER='"+orderNo+"')        "
		...
		+" LEFT JOIN IBMOE_BILLING_ACCOUNTS IBMOE_BILLING_ACCOUNTS ON IBMOE_BILLING_ACCOUNTS.CHILD_ID=IBMOE_ORDER_LINE.CHILD_ID "
		+" INNER JOIN IBMOE_ORDER_SERVICE_DETAILS SERVICE ON SERVICE.SERVICE_NO= IBMOE_ORDER_LINE.SERVICE_NO "
		+" WHERE IBMOE_ORDER_LINE.CRM_ORDER_ID NOT IN (SELECT maX(CRM_ORDER_ID) FROM IBMOE_ORDER_SERVICE_DETAILS WHERE LOGICAL_SI_NUMBER='"+orderNo+"')        "
...
/*	private static String strGetOrderLineFromCRM3="select  IBMOE_ORDER_LINE.ORDER_LINE_ID,IBMOE_ORDER_LINE.SERVICE_LIST_ID AS SERVICEID,IBMOE_ORDER_LINE.M6_PRODUCT_ID AS SERVICEDETAILID,"
	...
	private static String strGetOrderLineFromCRM3=" SELECT * FROM ("
		...
		+"  THEN NULL"
		+"  ELSE "
		+"  (SELECT MAX(SERVICE_NO) FROM IBMOE_ORDER_SERVICE_DETAILS WHERE LOGICAL_SI_NUMBER = '"+orderNo+"' AND SERVICE_NO < SERVICE.SERVICE_NO)"
		+"  END"
		+"  AS SERVICEID,"
		+" CASE WHEN IBMOE_ORDER_LINE.CRM_PRODUCT_NAME='Additional Billing Info' THEN IBMOE_ORDER_LINE.PARENT_SERVICE_KEY"
		+" ELSE IBMOE_ORDER_LINE.M6_PRODUCT_ID END"
		...
		+" CASE WHEN IBMOE_ORDER_LINE.LINE_STATUS = 'NEW' THEN null"
		+" ELSE (SELECT ORDERTYPE FROM IBMOE_ORDER_HEADER WHERE CRM_ORDER_ID=IBMOE_ORDER_LINE.CRM_ORDER_ID)"
		+" END AS CHANGETYPEID,"
        +" CASE WHEN IBMOE_ORDER_LINE.LINE_STATUS='NEW'" 
   		+" THEN NULL ELSE"
        +" IBMOE_ORDER_LINE.CRM_ORDER_ID"
		...
		+"  CASE WHEN IBMOE_ORDER_LINE.CRM_ORDER_ID=(SELECT MAX(IBSD.CRM_ORDER_ID) FROM IBMOE_ORDER_SERVICE_DETAILS  IBSD WHERE IBSD.LOGICAL_SI_NUMBER='"+orderNo+"' )"
		+"  THEN 0 "
		+"  ELSE    1 END"
		+"  AS IS_IN_HISTORY,     "
		+"  IBMOE_CIRCUIT_DETAIL.CREATION_DATE AS CREATED_DATE, "
	...
	private static String strGetOrderLineFromCRM4="SELECT "
		...
		+" WHEN IBMOE_ORDER_LINE.PARENT_SERVICE_KEY=42 AND IBMOE_ORDER_LINE.PARENT_SERVICE_KEY<>IBMOE_ORDER_LINE.CHILD_SERVICE_KEY"
		+" THEN ("
		+" SELECT OLC1.ORDER_LINE_ID FROM IBMOE_ORDER_LINE OLC1 WHERE OLC1.CRM_ORDER_ID=IBMOE_ORDER_LINE.CRM_ORDER_ID "
		...
		+" (SELECT OLC.IB_ORDER_LINE_ID FROM IBMOE_ORDER_LINE OLC"
		+" WHERE  OLC.CRM_ORDER_ID IN ("
		+" SELECT DISTINCT IBMOE_ORDER_SERVICE_DETAILS.CRM_ORDER_ID FROM IBMOE_ORDER_SERVICE_DETAILS WHERE LOGICAL_SI_NUMBER='1441854' "
		+" AND IBMOE_ORDER_SERVICE_DETAILS.CRM_ORDER_ID=IBMOE_ORDER_LINE.CRM_ORDER_ID"
		+" ) AND OLC.CHILD_SERVICE_KEY=(SELECT PARENT_SERVICE_KEY FROM IBMOE_ORDER_LINE  OL WHERE  OL.ORDER_LINE_ID=IBMOE_ORDER_LINE.ORDER_LINE_ID)"
		+" )"
		+" )END "
		...
		+" AS RESUME_IN_CURRENT_ORDER,"
		+" CASE WHEN IBMOE_ORDER_LINE.LINE_STATUS = 'NEW' THEN null"
		+" ELSE (SELECT ORDER_SUBTYPE FROM IBMOE_ORDER_HEADER WHERE CRM_ORDER_ID=IBMOE_ORDER_LINE.CRM_ORDER_ID)"
		+" END AS CHANGETYPEID,"
		+" CASE WHEN IBMOE_ORDER_LINE.LINE_STATUS = 'NEW' THEN null"
		+" ELSE SERVICE_ORDER_TYPE    "		
		+" END"
	...
	private static String strGetLineAttributesFromCRM2="SELECT IBMOE_CONFIG_PARAM.PRODUCT_ATTRIBUTE_ID,NULLIF(IBMOE_CONFIG_PARAM.M6_LABEL_VALUE,'0') AS M6_LABEL_VALUE,"
		...
		+" ELSE 'C' END"
		+" AS ORDERTYPE"
		+" FROM  APPS.IBMOE_CONFIG_PARAM IBMOE_CONFIG_PARAM"
		+" JOIN XXIBM.IBMOE_ORDER_LINE IBMOE_ORDER_LINE ON IBMOE_ORDER_LINE.ORDER_LINE_ID=IBMOE_CONFIG_PARAM.ORDER_LINE_ID"
...
*/
	...
	private static String strGetLineAttributesFromCRM3="SELECT * FROM("
		...
		+" FROM  APPS.IBMOE_CONFIG_PARAM IBMOE_CONFIG_PARAM"
		+" JOIN XXIBM.IBMOE_ORDER_LINE IBMOE_ORDER_LINE ON IBMOE_ORDER_LINE.ORDER_LINE_ID=IBMOE_CONFIG_PARAM.ORDER_LINE_ID"
		+" WHERE IBMOE_ORDER_LINE.CHILD_SERVICE_KEY NOT IN (1,33,42,59,88,133,138,142,149,1230) AND HIDDEN_FLAG='N' "
		+" AND IBMOE_ORDER_LINE.CRM_ORDER_ID "
		+" IN(SELECT MIN(CRM_ORDER_ID) FROM IBMOE_ORDER_SERVICE_DETAILS WHERE LOGICAL_SI_NUMBER='"+orderNo+"')"
		
		+" UNION"
		
		+" SELECT "
		+" IBMOE_CONFIG_PARAM.PRODUCT_ATTRIBUTE_ID,"
		+" CASE WHEN IBMOE_CONFIG_PARAM.ATTRIBUTE_STATUS='CHANGED' THEN"
		+" ("
		+" SELECT ICP2.M6_LABEL_VALUE FROM IBMOE_CONFIG_PARAM ICP2 WHERE ICP2.order_line_id IN("
		+" (SELECT MAX (ol1.order_line_id)"
		+" FROM ibmoe_order_line ol1"
		...
		+" )"
		+" ELSE NULLIF(IBMOE_CONFIG_PARAM.M6_LABEL_VALUE,'0')"
		+" END"
		+" AS ATTVALUE,"
		+" NULLIF(IBMOE_CONFIG_PARAM.M6_LABEL_VALUE,'0') AS ATTVALUE_NEW,"
		+" CASE WHEN IBMOE_ORDER_LINE.PARENT_SERVICE_KEY=IBMOE_ORDER_LINE.CHILD_SERVICE_KEY "
		...
		+" (SELECT ICP.ORDER_LINE_ID FROM IBMOE_CONFIG_PARAM  ICP  WHERE ICP.ATTRIBUTE_STATUS='CHANGED' and  "
		+" ICP.ORDER_LINE_ID IN("
		+" SELECT IOL.ORDER_LINE_ID FROM IBMOE_ORDER_LINE IOL WHERE IOL.CRM_ORDER_ID IN("
		+" 	(SELECT IOSD.CRM_ORDER_ID FROM IBMOE_ORDER_SERVICE_DETAILS IOSD WHERE IOSD.LOGICAL_SI_NUMBER='"+orderNo+"')"
		+"  )"
		+" )"
		+" )"
	...
	private static String strGetChargesHeaderFromCRM2="SELECT IBMOE_CHARGES_HDR.CRM_ORDER_ID,CHARGE_HDR_ID,PO_ID,IBMOE_CHARGES_HDR.CUST_ACCOUNT_ID,CREDIT_PERIOD,LEGAL_ENTITY_CODE,CASE WHEN BILLING_MODE = 'MANUAL' THEN 2 "
		...
		+"  FROM XXIBM.IBMOE_CHARGES_HDR IBMOE_CHARGES_HDR"
		+"  INNER JOIN IBMOE_ORDER_SERVICE_DETAILS SERVICE ON SERVICE.CRM_ORDER_ID= IBMOE_CHARGES_HDR.CRM_ORDER_ID  "
		+"  INNER JOIN IBMOE_ORDER_LINE IBMOE_ORDER_LINE ON IBMOE_ORDER_LINE.ORDER_LINE_ID=IBMOE_CHARGES_HDR.ORDER_LINE_ID "
	...
	private static String strGetHardwareHeaderFromCRM2="SELECT IBMOE_CHARGES_HDR.STORE,IBMOE_CHARGES_HDR.HARDWARE_TYPE,IBMOE_CHARGES_HDR.FORM_C_AVAILABLE,IBMOE_CHARGES_HDR.TYPE_OF_SALE,IBMOE_CHARGES_HDR.NATURE_OF_SALE,IBMOE_CHARGES_HDR.DISPATCH_ADDRESS_ID,"
		...
		+"  INNER JOIN IBMOE_ORDER_LINE IBMOE_ORDER_LINE ON IBMOE_ORDER_LINE.ORDER_LINE_ID=IBMOE_CHARGES_HDR.ORDER_LINE_ID "
		+"  WHERE IBMOE_CHARGES_HDR.CRM_ORDER_ID "
		+"  IN(SELECT MAX(CRM_ORDER_ID) FROM IBMOE_ORDER_SERVICE_DETAILS WHERE LOGICAL_SI_NUMBER='"+orderNo+"') AND STORE IS NOT NULL";
	...
	private static String strGetHardwareHeaderFromCRM3="SELECT SERVICE.SERVICE_NO AS MAIN_SERVICE_ID,IBMOE_CHARGES_HDR.STORE,IBMOE_CHARGES_HDR.HARDWARE_TYPE,IBMOE_CHARGES_HDR.FORM_C_AVAILABLE,IBMOE_CHARGES_HDR.TYPE_OF_SALE,IBMOE_CHARGES_HDR.NATURE_OF_SALE,IBMOE_CHARGES_HDR.DISPATCH_ADDRESS_ID,"
		...
		+"  WHERE IBMOE_CHARGES_HDR.CRM_ORDER_ID "
		+"  IN(SELECT MAX(CRM_ORDER_ID) FROM IBMOE_ORDER_SERVICE_DETAILS WHERE LOGICAL_SI_NUMBER='"+orderNo+"'"	
		+" AND CRM_ORDER_ID NOT IN(SELECT MAX(CRM_ORDER_ID) FROM IBMOE_ORDER_SERVICE_DETAILS WHERE LOGICAL_SI_NUMBER='"+orderNo+"')) AND STORE IS NOT NULL";
    ...
    +" IBMOE_ORDER_LINE.LAST_UPDATE_DATE, IBMOE_ORDER_LINE.LAST_UPDATED_BY"  
    +" FROM XXIBM.IBMOE_ORDER_LINE IBMOE_ORDER_LINE "
    +" JOIN IBMOE_ORDER_HEADER IBMOE_ORDER_HEADER"
    ...
    +" AND IBMOE_ORDER_LINE.CRM_ORDER_ID IN(SELECT MAX(CRM_ORDER_ID) FROM IBMOE_ORDER_SERVICE_DETAILS WHERE LOGICAL_SI_NUMBER ='"+orderNo+"')";
	...
	+" FROM XXIBM.IBMOE_ORDER_LINE IBMOE_ORDER_LINE JOIN IBMOE_ORDER_HEADER IBMOE_ORDER_HEADER"
	+" ON IBMOE_ORDER_HEADER.CRM_ORDER_ID=IBMOE_ORDER_LINE.CRM_ORDER_ID "
	+" INNER JOIN IBMOE_ORDER_SERVICE_DETAILS SERVICE ON SERVICE.CRM_ORDER_ID= IBMOE_ORDER_LINE.CRM_ORDER_ID"          
	...
	private static String spGetApprovalWorkFlowTOIOMS3="{CALL IOE.ECRM_INSERT_INTO_TTASKACTIONHISTORY(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}";
	...
	private static String strGetChargesFromCRM3="SELECT SERVICE.SERVICE_NO AS MAIN_SERVICE_ID,IBMOE_CHARGES.CHARGE_ID,IBMOE_CHARGES.PK_CHARGES_ID,"
	+" (SELECT MAX (OL1.ORDER_LINE_ID)"
	+" FROM IBMOE_ORDER_LINE OL1"
	...
	private static String strGetECRMFXAccountCreateFromIOMS_ForChange="select distinct FX_ACCOUNT_EXTERNAL_ID AS ACCTEXTERNALID,ORDERNO from ioe.TPOSERVICEDETAILS serdet"
									+" inner join ioe.TPOSERVICEMASTER servmast on servmast.SERVICEID=serdet.SERVICEID"
									+" inner join ioe.TSERVICETYPEDETAIL SERTYPEDET ON SERTYPEDET.SERVICEDETAILID=serdet.SERVICEDETAILID"
									+" where SERTYPEDET.SENDTOFX=1 and serdet.PARENT_SERVICEPRODUCTID<>0  AND servmast.LOGICAL_SI_NO in('1353787')";
	
	private static String strGetECRMFXServiceCreateFromIOMS_ForChange="SELECT FX.SERVICE_INTERNAL_ID AS SUBSCRNO,FX.SERVICE_INTER_ID_RESET AS SUBSCRNORESET,FX.UPDATED_ON AS LASTUPDATEDDATE ,FX.CREATED_ON AS CREATEDDATE,FX.CRM_ORDER_ID AS ORDERNO,FX.order_line_id,(SELECT MAX (ol1.order_line_id) FROM "+tableSchemaName+"ibmoe_order_line ol1 WHERE ol1.ib_order_line_id =ol.ib_order_line_id) AS SERVICEPRODUCTID,FX.LEGAL_ENTITY AS ACCTEXTERNALID, FX.FX_TOKEN_NO AS TOKEN_ID,FX.SERVICE_LIST_ID AS CREATEDIN_SERVICEID,FX.SERVICE_VIEW_ID AS VIEW_ID,CIRCUIT.BILLING_TRIG_DATE AS SERVICEACTIVEDT,0 as AUTOBILLING, 3 AS  FX_SCHEDULAR_CREATE_STATUS,101 AS EMFCONFIGID FROM "+tableSchemaName+"IBMOE_CRM_FX_INTEGRATION FX INNER JOIN "+tableSchemaName+"ibmoe_order_line OL ON FX.ORDER_LINE_ID=OL.order_line_id LEFT JOIN "+tableSchemaName+"IBMOE_CIRCUIT_DETAIL CIRCUIT on FX.ORDER_LINE_ID = CIRCUIT.ORDER_LINE_ID where FX.charge_flag = 'C' and FX.SERVICE_LIST_ID in (SELECT SERVICE_LIST_ID FROM "+tableSchemaName+"IBMOE_ORDER_SERVICE_DETAILS WHERE LOGICAL_SI_NUMBER IN" +
			"('1445910'))";


	
	private static String strGetECRMFXRCCreateFromIOMS_ForChange="select FX.CRM_ORDER_ID AS ORDER_NO," +
			"FX.order_line_id,"+
			" (SELECT MAX (ol1.order_line_id)"+
            	" FROM "+tableSchemaName+"ibmoe_order_line ol1"+
					"  WHERE ol1.ib_order_line_id ="+
                                     "ol.ib_order_line_id) AS SERVICEPRODUCTID,"+  
						" CHARGES.pk_charges_id,"+

						" (SELECT MIN (pk_charges_id)"+
          						" FROM "+tableSchemaName+"ibmoe_charges c1"+
         							" WHERE c1.ib_pk_charges_id = CHARGES.ib_pk_charges_id)  AS CHARGEINFOID,"+

         							" FX.FX_VIEW_ID AS VIEWID,FX.LEGAL_ENTITY AS FX_EXT_ACCOUNT_NO,"+
         							" FX.UPDATED_ON AS LAST_MODIFIED_DATE,FX.CREATED_ON AS CREATEDDATE,"+
         							" CHARGE_MAP.ELEMENT_ID AS ELEMENTID,CHARGES.CHARGE_START_DATE AS BILLINGACTIVEDT,"+
         							" CHARGES.BILL_PERIOD AS BILLPERIOD ,ROUND(CHARGES.CHARGE_VALUE,2) AS OVERRIDE_RATE,"+
         							" FX.FX_TOKEN_NO AS TOKEN_ID,FX.SERVICE_LIST_ID AS CREATEDIN_SERVICEID,"+
         							" 0 AS AUTOBILLING, 3 AS FX_SCHEDULAR_CREATE_STATUS"+
         							" from "+tableSchemaName+"IBMOE_CRM_FX_INTEGRATION FX "+
         							" INNER JOIN "+tableSchemaName+"ibmoe_order_line OL ON FX.ORDER_LINE_ID=OL.order_line_id"+
         							" LEFT JOIN "+tableSchemaName+"IBMOE_CHARGES CHARGES on FX.PK_CHARGES_ID = CHARGES.PK_CHARGES_ID"+ 
         							" LEFT JOIN "+tableSchemaName+"IBMOE_CHARGE_MAP CHARGE_MAP on CHARGES.CHARGE_ID = CHARGE_MAP.CHARGE_ID"+ 
         							" where FX.charge_flag = 'Y' and FX.PK_CHARGES_ID is not null and FX.CHARGE_TYPE=30 and FX.SERVICE_LIST_ID in (SELECT SERVICE_LIST_ID FROM "+tableSchemaName+"IBMOE_ORDER_SERVICE_DETAILS WHERE LOGICAL_SI_NUMBER IN('1353787'))";
	
	private static String strGetECRMFXNRCCreateFromIOMS_ForChange="select FX.order_line_id,"+
									" (SELECT MAX (ol1.order_line_id)"+
									" FROM "+tableSchemaName+"ibmoe_order_line ol1"+
									" WHERE ol1.ib_order_line_id ="+
                                     " ol.ib_order_line_id) AS SERVICEPRODUCTID,"+  
                                     ...
                                     " FX.FX_VIEW_ID AS VIEWID,FX.LEGAL_ENTITY AS FX_EXT_ACCOUNT_NO,"+
                                     " FX.UPDATED_ON AS LAST_MODIFIED_DATE,FX.CREATED_ON AS CREATEDDATE,"+
                                     " FX.CRM_ORDER_ID AS ORDERNO,CHARGE_MAP.TYPE_ID AS TYPEIDNRC,CHARGES.charge_start_date AS EFFECTIVE_DATE,"+
                                     " ROUND(CHARGES.CHARGE_VALUE,2) AS NRC_AMOUNT,FX_TOKEN_NO AS TOKEN_ID ,FX.SERVICE_LIST_ID AS CREATEDIN_SERVICEID,"+
                                     " 3 AS FX_SCHEDULAR_CREATE_STATUS,0 AS AUTOBILLING"+
                                     " from "+tableSchemaName+"IBMOE_CRM_FX_INTEGRATION FX"+
                                     " INNER JOIN "+tableSchemaName+"ibmoe_order_line OL ON FX.ORDER_LINE_ID=OL.order_line_id"+
                                     " LEFT JOIN "+tableSchemaName+"IBMOE_CHARGES CHARGES on FX.PK_CHARGES_ID = CHARGES.PK_CHARGES_ID"+
                                     " LEFT JOIN "+tableSchemaName+"IBMOE_CHARGE_MAP CHARGE_MAP on CHARGES.CHARGE_ID = CHARGE_MAP.CHARGE_ID"+
                                     " where FX.charge_flag = 'Y' and FX.PK_CHARGES_ID is not null and FX.CHARGE_TYPE=20 and FX.SERVICE_LIST_ID in (SELECT SERVICE_LIST_ID FROM "+tableSchemaName+"IBMOE_ORDER_SERVICE_DETAILS WHERE LOGICAL_SI_NUMBER IN('1353787'))";
	
	private static String strGetECRMFXAUTOBILLING="";


	private static String strGetECRMFXCHARGEDISCONNECTION="select CHARGES.pk_charges_id,"+
									"(SELECT MIN (pk_charges_id)"+
									" FROM "+tableSchemaName+"ibmoe_charges c1"+
									" WHERE c1.ib_pk_charges_id = CHARGES.ib_pk_charges_id)  AS CHARGEINFOID,"+
									" CHARGES.charge_end_date as DISCONNECTION_DATE,"+
									" CASE WHEN CHARGES.charge_type=30 THEN 1 when  CHARGES.charge_type=20 then 2 end AS CHARGE_TYPE,3 AS PROCESSING_STATUS,"+
									" FX.FX_VIEW_ID AS VIEW_ID,2 AS DISCONNECTION_STEP,FX.CREATED_ON AS CREATED_DATE,"+
									" FX.UPDATED_ON AS LAST_UPDATED_DATE,FX.FX_TOKEN_NO AS TOKEN_ID,CIRCUIT.BILLING_TRIG_DATE as BILLING_TRIGGER_DATE,"+
									" FX.SERVICE_LIST_ID as DISCONNECTEDIN_SERVICEID,FX.CRM_ORDER_ID AS ORDERNO"+
									" from "+tableSchemaName+"IBMOE_CRM_FX_INTEGRATION FX "+
									" LEFT JOIN "+tableSchemaName+"IBMOE_CIRCUIT_DETAIL CIRCUIT on FX.ORDER_LINE_ID = CIRCUIT.ORDER_LINE_ID"+
									" LEFT JOIN "+tableSchemaName+"IBMOE_CHARGES CHARGES on FX.PK_CHARGES_ID = CHARGES.PK_CHARGES_ID "+
									" where FX.charge_flag = 'E' and FX.PK_CHARGES_ID is not null and FX.SERVICE_LIST_ID in (SELECT SERVICE_LIST_ID FROM "+tableSchemaName+"IBMOE_ORDER_SERVICE_DETAILS WHERE LOGICAL_SI_NUMBER IN('1353787'))";

	
	
	private static String strGetECRMFXSERVICEUPDATE="SELECT 3 AS  FX_SCHEDULAR_UPDATE_STATUS,FX.CREATED_ON AS CREATEDDATE,FX.ORDER_LINE_ID,"
								+" (SELECT MAX (ol1.order_line_id)"
								+" FROM "+tableSchemaName+"ibmoe_order_line ol1"
								+" WHERE ol1.ib_order_line_id ="
								+" ol.ib_order_line_id) AS SERVICEPRODUCTID,"
								+" FX.UPDATED_ON AS LAST_MODIFIED_DATE,FX.SERVICE_LIST_ID AS UPDATED_IN_SERVICEID,CIRCUIT.BILLING_TRIG_DATE AS SERVICEACTIVEDT,"
								+" FX.CRM_ORDER_ID AS ORDERNO,'ALL' AS FIELDS_FOR_VALIDATION"
								+" FROM "+tableSchemaName+"IBMOE_CRM_FX_INTEGRATION FX"
								+" INNER JOIN "+tableSchemaName+"ibmoe_order_line OL ON FX.ORDER_LINE_ID=OL.order_line_id"
								+" LEFT JOIN "+tableSchemaName+"IBMOE_CIRCUIT_DETAIL CIRCUIT on FX.ORDER_LINE_ID = CIRCUIT.ORDER_LINE_ID"
								+" where FX.charge_flag = 'U' and FX.SERVICE_LIST_ID in (SELECT SERVICE_LIST_ID FROM "+tableSchemaName+"IBMOE_ORDER_SERVICE_DETAILS WHERE LOGICAL_SI_NUMBER IN('1445910'))";

	
	private static String strGetECRMFXSERVICEDISCONNECTION="SELECT FX.SERVICE_LIST_ID AS DISCONNECTEDIN_SERVICEID,FX.order_line_id,"
										+" (SELECT MAX (ol1.order_line_id)"
										+" FROM "+tableSchemaName+"ibmoe_order_line ol1"
										+" WHERE ol1.ib_order_line_id ="
										+" ol.ib_order_line_id) AS SERVICEPRODUCTID,"
										+" CIRCUIT.BILLING_TRIG_DATE AS DISCONNECTION_DATE,CIRCUIT.BILLING_TRIG_DATE AS BILLINGTRIGGERDATE,"
										+" 3 AS PROCESSING_STATUS,FX.CREATED_ON AS CREATED_DATE,FX.UPDATED_ON AS LAST_UPDATED_DATE," 
										+" FX.FX_TOKEN_NO AS TOKEN_ID,FX.CRM_ORDER_ID AS DISCONNECTEDIN_ORDER_NO,FX.CRM_ORDER_ID AS ORDERNO"
										+" FROM "+tableSchemaName+"IBMOE_CRM_FX_INTEGRATION FX"
										+" INNER JOIN "+tableSchemaName+"ibmoe_order_line OL ON FX.ORDER_LINE_ID=OL.order_line_id"
										+" LEFT JOIN "+tableSchemaName+"IBMOE_CIRCUIT_DETAIL CIRCUIT on FX.ORDER_LINE_ID = CIRCUIT.ORDER_LINE_ID"
										+" where FX.charge_flag = 'D' and FX.SERVICE_LIST_ID in (SELECT SERVICE_LIST_ID FROM "+tableSchemaName+"IBMOE_ORDER_SERVICE_DETAILS WHERE LOGICAL_SI_NUMBER IN('1353787'))";


	private static String strInsertECRMFXAccountToCreate_ForChange="{CALL IOE.ECRM_INSERT_INTO_TFX_ACCOUNTCREATE_2(?,?,?,?,?)}";
	private static String strInsertECRMFXServiceCreateToIOMS_ForChange="{CALL IOE.ECRM_INSERT_INTO_TFX_SERVICECREATE_2(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}";
	
	private static String strInsertECRMFXRCCreateToIOMS_ForChange="{CALL IOE.ECRM_INSERT_INTO_TFX_RC_CREATE_2(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}";
	private static String strInsertECRMFXNRCCreateToIOMS_ForChange="{CALL IOE.ECRM_INSERT_INTO_TFX_NRC_CREATE_2(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}";
	private static String strInsertECRMFXAUTOBILLING="{CALL IOE.ECRM_INSERT_INTO_TAUTOBILLING(?,?,?,?,?,?,?,?,?,?,?)}";
	private static String strInsertECRMFXCHARGEDISCONNECT="{CALL IOE.ECRM_INSERT_INTO_TFX_DISCONNECTION(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}";
	private static String strInsertECRMFXSERVICEUPDATE="{CALL IOE.ECRM_INSERT_INTO_TFX_SERVICEUPDATE(?,?,?,?,?,?,?,?,?,?,?)}";
	private static String strInsertECRMFXSERVICEDISCONNECT="{CALL IOE.ECRM_INSERT_INTO_TFX_SERVICE_DISCONNECTION(?,?,?,?,?,?,?,?,?,?,?,?,?)}";
...
*/			
	...
	*/			}
	...
	public static void InsertECRMOrderLinetoIOMS2()//INSERT INTO TPOSERVICEDETAILS
	{
		PreparedStatement pstmt = null;
		ResultSet rset = null;
		DBConnectionRetriever con = new DBConnectionRetriever();
	...
	public static void InsertECRMLineAttributestoIOMS3()//INSERT INTO TPRODUCTLINEATTVALUE
	{
		PreparedStatement pstmt = null;
		ResultSet rset = null;
		DBConnectionRetriever con = new DBConnectionRetriever();
	...
	public static void InsertECRMLocationInfotoIOMS2()//insert into TLOCATION_INFO
	{
		PreparedStatement pstmt = null;
		ResultSet rset = null;
		DBConnectionRetriever con = new DBConnectionRetriever();
	...
	public static void InsertECRMChargestoIOMS2()
	{
		PreparedStatement pstmt = null;
		ResultSet rset = null;
		DBConnectionRetriever con = new DBConnectionRetriever();
		...
		try {
			...
			while (rset.next()) 
			{
				...
				objDto.setCrmChargeId1(rset.getString("CHARGE_ID"));
				objDto.setChargeId(rset.getString("PK_CHARGES_ID"));
				objDto.setOrderLineId(rset.getString("ORDER_LINE_ID"));
				objDto.setChargetype(rset.getString("CHARGE_TYPE"));
				objDto.setContractPeriodMonths(rset.getString("CONTRACT_PERIOD_MNTHS"));
			...
			csIOMS.setString(26, element.getCharge_start_date());
			csIOMS.setString(27, element.getCharge_end_date());
			csIOMS.setString(28, element.getExclude());
			csIOMS.setString(29, element.getAnnual_rate());
			csIOMS.setString(30, element.getCharge_status());
			csIOMS.setString(31, element.getCrmOrderId());
			csIOMS.setString(32, element.getServiceNo());
			csIOMS.setString(33, element.getCrmChargeId1());
			csIOMS.execute();
			}
	...
	public static void InsertECRMChargestoIOMS3()
	{
		PreparedStatement pstmt = null;
		ResultSet rset = null;
		DBConnectionRetriever con = new DBConnectionRetriever();
		...
		try {
			...
			csIOMS.setString(32, element.getServiceNo());
			csIOMS.setString(33, element.getCrmChargeId1());
			csIOMS.setString(34, element.getMainServiceId());
...
//	 adde by manisha START	
	
	public static void InsertECRMFXAccountCreationtoIOMS_Change()//INSERT INTO IOE.TFX_ACCOUNTCREATE
	{
		PreparedStatement pstmt = null;
		ResultSet rset = null;
		DBConnectionRetriever con = new DBConnectionRetriever();
		...
		try {
			System.out.println("Connect with IOMS database");
			iomsConn = getConnectionObject();
			System.out.println("query ::" + strGetECRMFXAccountCreateFromIOMS_ForChange);
			pstmt = iomsConn.prepareStatement(strGetECRMFXAccountCreateFromIOMS_ForChange);
			//pstmt.setString(1,insertDate);
			rset = pstmt.executeQuery();
			int i = 0;
			ArrayList lstM6Task = new ArrayList();
			...
			{
				objDto = new ECRMMigrationDto();
				objDto.setLegalEntityCode(rset.getString("ACCTEXTERNALID"));
				objDto.setCrmOrderId(rset.getString("ORDERNO"));
				lstM6Task.add(objDto);
			...
			System.err.println("FX Account Create Information And Stored In ArrayList");	
			iomsConn.setAutoCommit(false);
			csIOMS = iomsConn.prepareCall(strInsertECRMFXAccountToCreate_ForChange);
	...
	public static void InsertECRMFXServiceCreationtoIOMS_Change()//INSERT INTO IOE.TFX_SERVICECREATE
	{
		PreparedStatement pstmt = null;
		ResultSet rset = null;
		DBConnectionRetriever con = new DBConnectionRetriever();
		...
		try {
			System.out.println("Connect with IOMS database");
			iomsConn = getConnectionObject();
			System.out.println("query ::" + strGetECRMFXServiceCreateFromIOMS_ForChange);
			pstmt = crmcon.prepareStatement(strGetECRMFXServiceCreateFromIOMS_ForChange);
			//pstmt.setString(1,insertDate);
			rset = pstmt.executeQuery();
			int i = 0;
			ArrayList lstM6Task = new ArrayList();
			...
			{
				objDto = new ECRMMigrationDto();
				objDto.setSubScrNo(rset.getString("SUBSCRNO"));
				objDto.setSubScrNoReset(rset.getString("SUBSCRNORESET"));
				objDto.setLastUpdatedDate1(rset.getString("LASTUPDATEDDATE"));
				objDto.setCreationDate1(rset.getString("CREATEDDATE"));
				objDto.setCrmOrderId(rset.getString("ORDERNO"));
				objDto.setOrderLineId(rset.getString("SERVICEPRODUCTID"));
				objDto.setAccExternalId(rset.getString("ACCTEXTERNALID"));
				objDto.setTokenId(rset.getString("TOKEN_ID"));
				objDto.setServiceNo(rset.getString("CREATEDIN_SERVICEID"));
				objDto.setViewId(rset.getString("VIEW_ID"));
				objDto.setServiceActiveDT(rset.getString("SERVICEACTIVEDT"));
				objDto.setAutobilling(rset.getInt("AUTOBILLING"));
				objDto.setFx_scheduler_status(rset.getInt("FX_SCHEDULAR_CREATE_STATUS"));
				objDto.setEmfconfigid(rset.getInt("EMFCONFIGID"));
				lstM6Task.add(objDto);
			}
			...
			System.err.println("FX Service Create Information Stored In ArrayList");	
			iomsConn.setAutoCommit(false);
			csIOMS = iomsConn.prepareCall(strInsertECRMFXServiceCreateToIOMS_ForChange);
			...
			for (Iterator iter = lstM6Task.iterator(); iter.hasNext();) 
			{
				...
				csIOMS.setString(10,element.getViewId());
				csIOMS.setString(11,element.getServiceActiveDT());
				csIOMS.setInt(12,element.getAutobilling());
				csIOMS.setInt(13,element.getFx_scheduler_status());
				csIOMS.setInt(14,element.getEmfconfigid());
				csIOMS.setInt(15, 0);
				csIOMS.setInt(16, 0);
				csIOMS.setString(17, "");
				csIOMS.execute();
	...
	public static void InsertECRMFXRCCreationtoIOMS_Change()//INSERT INTO IOE.TFX_RC_CREATE
	{
		PreparedStatement pstmt = null;
		ResultSet rset = null;
		DBConnectionRetriever con = new DBConnectionRetriever();
		...
		try {
			System.out.println("Connect with IOMS database");
			iomsConn = getConnectionObject();
			System.out.println("query ::" + strGetECRMFXRCCreateFromIOMS_ForChange);
			pstmt = crmcon.prepareStatement(strGetECRMFXRCCreateFromIOMS_ForChange);
			//pstmt.setString(1,insertDate);
			rset = pstmt.executeQuery();
			int i = 0;
			ArrayList lstM6Task = new ArrayList();
			...
			{
				objDto = new ECRMMigrationDto();
				objDto.setCrmOrderId(rset.getString("ORDER_NO"));
				objDto.setOrderLineId(rset.getString("SERVICEPRODUCTID"));
				objDto.setChargeId(rset.getString("CHARGEINFOID"));
				objDto.setViewId(rset.getString("VIEWID"));
				objDto.setLegalEntityCode(rset.getString("FX_EXT_ACCOUNT_NO"));
				objDto.setLastUpdatedDate1(rset.getString("LAST_MODIFIED_DATE"));
				objDto.setCreationDate1(rset.getString("CREATEDDATE"));
				objDto.setElementid(rset.getString("ELEMENTID"));
				objDto.setBillingTriggerDate(rset.getString("BILLINGACTIVEDT"));
				objDto.setBill_period(rset.getString("BILLPERIOD"));
				objDto.setOverride_rate(rset.getString("OVERRIDE_RATE"));
				objDto.setTokenId(rset.getString("TOKEN_ID"));
				objDto.setServiceNo(rset.getString("CREATEDIN_SERVICEID"));
				objDto.setAutobilling(rset.getInt("AUTOBILLING"));
				objDto.setFx_scheduler_status(rset.getInt("FX_SCHEDULAR_CREATE_STATUS"));
			...
			System.err.println("RC Fetched and Stored In ArrayList");	
			iomsConn.setAutoCommit(false);
			csIOMS = iomsConn.prepareCall(strInsertECRMFXRCCreateToIOMS_ForChange);
			...
			for (Iterator iter = lstM6Task.iterator(); iter.hasNext();) 
			{
				...
				csIOMS.setString(6, element.getLastUpdatedDate1());
				csIOMS.setString(7, element.getCreationDate1());
				csIOMS.setString(8, element.getElementid());
				csIOMS.setString(9, element.getBillingTriggerDate());
				csIOMS.setString(10, element.getBill_period());
				csIOMS.setString(11, element.getOverride_rate());
				csIOMS.setString(12, element.getTokenId());
				csIOMS.setString(13, element.getServiceNo());
				csIOMS.setInt(14, element.getAutobilling());
				csIOMS.setInt(15, element.getFx_scheduler_status());
				csIOMS.setInt(16, 0);
				csIOMS.setInt(17, 0);
				csIOMS.setString(18, "");
				csIOMS.execute();
	...
	public static void InsertECRMFXNRCCreationtoIOMS_Change()//INSERT INTO IOE.TFX_NRC_CREATE
	{
		PreparedStatement pstmt = null;
		ResultSet rset = null;
		DBConnectionRetriever con = new DBConnectionRetriever();
		...
		try {
			System.out.println("Connect with IOMS database");
			iomsConn = getConnectionObject();
			System.out.println("query ::" + strGetECRMFXNRCCreateFromIOMS_ForChange);
			pstmt = crmcon.prepareStatement(strGetECRMFXNRCCreateFromIOMS_ForChange);
			//pstmt.setString(1,insertDate);
			rset = pstmt.executeQuery();
			int i = 0;
			ArrayList lstM6Task = new ArrayList();
			...
			while (rset.next()) 
			{
				...
				objDto.setCreationDate1(rset.getString("CREATEDDATE"));
				objDto.setCrmOrderId(rset.getString("ORDERNO"));
				objDto.setElementid(rset.getString("TYPEIDNRC"));
				objDto.setBillingTriggerDate(rset.getString("EFFECTIVE_DATE"));
				objDto.setOverride_rate(rset.getString("NRC_AMOUNT"));
				objDto.setTokenId(rset.getString("TOKEN_ID"));
				objDto.setServiceNo(rset.getString("CREATEDIN_SERVICEID"));
				objDto.setFx_scheduler_status(rset.getInt("FX_SCHEDULAR_CREATE_STATUS"));
				objDto.setAutobilling(rset.getInt("AUTOBILLING"));
			...
			System.err.println("NRC Charge Created And Stored In ArrayList");	
			iomsConn.setAutoCommit(false);
			csIOMS = iomsConn.prepareCall(strInsertECRMFXNRCCreateToIOMS_ForChange);
			...
			for (Iterator iter = lstM6Task.iterator(); iter.hasNext();) 
			{
				...
				csIOMS.setString(8, element.getElementid());
				csIOMS.setString(9, element.getBillingTriggerDate());
				csIOMS.setString(10, element.getOverride_rate());
				csIOMS.setString(11, element.getTokenId());
				csIOMS.setString(12, element.getServiceNo());
				csIOMS.setInt(13, element.getFx_scheduler_status());
				csIOMS.setInt(14, element.getAutobilling());
				csIOMS.setInt(15, 0);
				csIOMS.setInt(16, 0);
	...
	public static void InsertECRMFXTAUTOBILLING()//INSERT INTO IOE.TAUTO_BILLING
	{
		PreparedStatement pstmt = null;
		ResultSet rset = null;
		DBConnectionRetriever con = new DBConnectionRetriever();
		...
		try {
			System.out.println("Connect with IOMS database");
			iomsConn = getConnectionObject();
			System.out.println("query ::" + strGetECRMFXAUTOBILLING);
			pstmt = crmcon.prepareStatement(strGetECRMFXAUTOBILLING);
			//pstmt.setString(1,insertDate);
			rset = pstmt.executeQuery();
			int i = 0;
			ArrayList lstM6Task = new ArrayList();
			...
			{
				objDto = new ECRMMigrationDto();
				objDto.setOrderLineId(rset.getString("SPID"));
				objDto.setFxStatus(rset.getString("STATUS"));
				objDto.setCrmOrderId(rset.getString("ORDERNO"));
				objDto.setServiceNo(rset.getString("SERVICEID"));
				objDto.setCreatedDate(rset.getString("CURRENT_DATE"));
				objDto.setLocNo(rset.getString("LOCNO"));
				objDto.setLocDate(rset.getString("LOCDATE"));
				objDto.setFx_scheduler_status(rset.getInt("FX_PUSH_STATUS"));
				lstM6Task.add(objDto);
			...
			if(lstM6Task.size()>0)
			{
			System.err.println("Auto billing Data Created And Stored In ArrayList");	
			iomsConn.setAutoCommit(false);
			csIOMS = iomsConn.prepareCall(strInsertECRMFXAUTOBILLING);
			...
			for (Iterator iter = lstM6Task.iterator(); iter.hasNext();) 
			{
				...
				csIOMS.setString(1, element.getOrderLineId());
				csIOMS.setString(2, element.getFxStatus());
				csIOMS.setString(3, element.getCrmOrderId());
				csIOMS.setString(4, element.getServiceNo());
				csIOMS.setString(5, element.getCreatedDate());
				csIOMS.setString(6, element.getLocNo());
				csIOMS.setString(7, element.getLocDate());
				csIOMS.setInt(8, element.getFx_scheduler_status());
				csIOMS.setInt(9, 0);
				csIOMS.setInt(10, 0);
				csIOMS.setString(11, "");
				csIOMS.execute();
			...
			if (csIOMS.getInt(10) == 0) {
				...
				//UpdateIOMSDataTracker("insert","TM_ACCOUNTROLEDETAILS",sdf.format(new Date()),sdf.format(new Date()));
				isInserted = true;
				System.err.println("Auto Billing Details Updated....");
			} else {
				System.err.println("Auto Billing Details Not Updated....");
				System.err.println(csIOMS.getString(8));
				iomsConn.rollback();
				isInserted = false;
			}
				...
				// Long ServiceSegment = null;
			System.out.println("Auto Billing Details Inserted");
			} catch (Exception e) {
			System.out.println("Error in method AutoBilling"
					+ e.getStackTrace());
			e.printStackTrace();
		} finally {
			try {
	...
	public static void InsertECRMFXDISCONNECTIONCHARGE()//INSERT INTO IOE.TFX_DISCONNECTION
	{
		PreparedStatement pstmt = null;
		ResultSet rset = null;
		DBConnectionRetriever con = new DBConnectionRetriever();
		...
		try {
			System.out.println("Connect with IOMS database");
			iomsConn = getConnectionObject();
			System.out.println("query ::" + strGetECRMFXCHARGEDISCONNECTION);
			pstmt = crmcon.prepareStatement(strGetECRMFXCHARGEDISCONNECTION);
			//pstmt.setString(1,insertDate);
			rset = pstmt.executeQuery();
			int i = 0;
			ArrayList lstM6Task = new ArrayList();
			...
			while (rset.next()) 
			{
				objDto = new ECRMMigrationDto();
				objDto.setChargeId(rset.getString("CHARGEINFOID"));
				objDto.setDisconnection_date(rset.getString("DISCONNECTION_DATE"));
				objDto.setChargetype(rset.getString("CHARGE_TYPE"));
				objDto.setFx_scheduler_status(rset.getInt("PROCESSING_STATUS"));
				objDto.setViewId(rset.getString("VIEW_ID"));
				objDto.setDisconnection_step(rset.getString("DISCONNECTION_STEP"));
				objDto.setCreatedDate(rset.getString("CREATED_DATE"));
				objDto.setLastUpdatedDate1(rset.getString("LAST_UPDATED_DATE"));
				objDto.setTokenId(rset.getString("TOKEN_ID"));
				objDto.setBillingTriggerDate(rset.getString("BILLING_TRIGGER_DATE"));
				objDto.setServiceNo(rset.getString("DISCONNECTEDIN_SERVICEID"));
				objDto.setCrmOrderId(rset.getString("ORDERNO"));
				lstM6Task.add(objDto);
			...
			if(lstM6Task.size()>0)
			{
			System.err.println("Disconnect Charge Created And Stored In ArrayList");	
			iomsConn.setAutoCommit(false);
			csIOMS = iomsConn.prepareCall(strInsertECRMFXCHARGEDISCONNECT);
			...
			for (Iterator iter = lstM6Task.iterator(); iter.hasNext();) 
			{
				...
				csIOMS.setString(1, element.getChargeId());
				csIOMS.setString(2, element.getDisconnection_date());
				csIOMS.setString(3, element.getChargetype());
				csIOMS.setInt(4, element.getFx_scheduler_status());
				csIOMS.setString(5, element.getViewId());
				csIOMS.setString(6, element.getDisconnection_step());
				csIOMS.setString(7, element.getCreatedDate());
				csIOMS.setString(8, element.getLastUpdatedDate1());
				csIOMS.setString(9, element.getTokenId());
				csIOMS.setString(10, element.getBillingTriggerDate());
				csIOMS.setString(11, element.getServiceNo());
				csIOMS.setString(12, element.getCrmOrderId());
				csIOMS.setInt(13, 0);
				csIOMS.setInt(14, 0);
				csIOMS.setString(15, "");
			...
			if (csIOMS.getInt(14) == 0) {
				iomsConn.commit();
				//SimpleDateFormat sdf=new SimpleDateFormat("MM/dd/yyyy");									
				//UpdateIOMSDataTracker("insert","TM_ACCOUNTROLEDETAILS",sdf.format(new Date()),sdf.format(new Date()));
				isInserted = true;
				System.err.println("Disconnect Charge Details Updated....");
			} else {
				System.err.println("Disconnect Charge Details Not Updated....");
				System.err.println(csIOMS.getString(8));
				iomsConn.rollback();
				isInserted = false;
			}
				...
				// Long ServiceSegment = null;
			System.out.println("Disconnect Charge Details Inserted");
			} catch (Exception e) {
			System.out.println("Error in method DisconnectCharge"
					+ e.getStackTrace());
			e.printStackTrace();
		} finally {
			try {
	...
	public static void InsertECRMFXSERVICEUPDATE()//INSERT INTO IOE.TFX_SERVICEUPDATE
	{
		PreparedStatement pstmt = null;
		ResultSet rset = null;
		DBConnectionRetriever con = new DBConnectionRetriever();
		...
		try {
			System.out.println("Connect with IOMS database");
			iomsConn = getConnectionObject();
			System.out.println("query ::" + strGetECRMFXSERVICEUPDATE);
			pstmt = crmcon.prepareStatement(strGetECRMFXSERVICEUPDATE);
			//pstmt.setString(1,insertDate);
			rset = pstmt.executeQuery();
			int i = 0;
			ArrayList lstM6Task = new ArrayList();
			...
			{
				objDto = new ECRMMigrationDto();
				objDto.setFx_scheduler_status(rset.getInt("FX_SCHEDULAR_UPDATE_STATUS"));
				objDto.setCreatedDate(rset.getString("CREATEDDATE"));
				objDto.setOrderLineId(rset.getString("SERVICEPRODUCTID"));
				objDto.setLastUpdatedDate1(rset.getString("LAST_MODIFIED_DATE"));
				objDto.setServiceNo(rset.getString("UPDATED_IN_SERVICEID"));
				objDto.setServiceActiveDT(rset.getString("SERVICEACTIVEDT"));
				objDto.setCrmOrderId(rset.getString("ORDERNO"));
				objDto.setField_for_validation(rset.getString("FIELDS_FOR_VALIDATION"));
				lstM6Task.add(objDto);
			...
			if(lstM6Task.size()>0)
			{
			System.err.println("Service Updated And Stored In ArrayList");	
			iomsConn.setAutoCommit(false);
			csIOMS = iomsConn.prepareCall(strInsertECRMFXSERVICEUPDATE);
			...
			for (Iterator iter = lstM6Task.iterator(); iter.hasNext();) 
			{
				...
				csIOMS.setInt(1, element.getFx_scheduler_status());
				csIOMS.setString(2, element.getCreatedDate());
				csIOMS.setString(3, element.getOrderLineId());
				csIOMS.setString(4, element.getLastUpdatedDate1());
				csIOMS.setString(5, element.getServiceNo());
				csIOMS.setString(6, element.getServiceActiveDT());
				csIOMS.setString(7, element.getCrmOrderId());
				csIOMS.setString(8, element.getField_for_validation());
				csIOMS.setInt(9, 0);
				csIOMS.setInt(10, 0);
				csIOMS.setString(11, "");
				csIOMS.execute();
			...
			if (csIOMS.getInt(10) == 0) {
				...
				//UpdateIOMSDataTracker("insert","TM_ACCOUNTROLEDETAILS",sdf.format(new Date()),sdf.format(new Date()));
				isInserted = true;
				System.err.println("ServiceUpdate Details Updated....");
			} else {
				System.err.println("ServiceUpdate Details Not Updated....");
				System.err.println(csIOMS.getString(8));
				iomsConn.rollback();
				isInserted = false;
			}
				...
				// Long ServiceSegment = null;
			System.out.println("ServiceUpdate Details Inserted");
			} catch (Exception e) {
			System.out.println("Error in method ServiceUpdate"
					+ e.getStackTrace());
			e.printStackTrace();
		} finally {
			try {
	...
	public static void InsertECRMFXSERVICEDISCONNECT()//INSERT INTO IOE.TFX_SERVICE_DISCONNECTION
	{
		PreparedStatement pstmt = null;
		ResultSet rset = null;
		DBConnectionRetriever con = new DBConnectionRetriever();
		...
		try {
			System.out.println("Connect with IOMS database");
			iomsConn = getConnectionObject();
			System.out.println("query ::" + strGetECRMFXSERVICEDISCONNECTION);
			pstmt = crmcon.prepareStatement(strGetECRMFXSERVICEDISCONNECTION);
			//pstmt.setString(1,insertDate);
			rset = pstmt.executeQuery();
			int i = 0;
			ArrayList lstM6Task = new ArrayList();
			...
			while (rset.next()) 
			{
				...
				objDto.setLastUpdatedDate1(rset.getString("LAST_UPDATED_DATE"));
				objDto.setTokenId(rset.getString("TOKEN_ID"));
				objDto.setDisc_orderno(rset.getString("DISCONNECTEDIN_ORDER_NO"));
				objDto.setCrmOrderId(rset.getString("ORDERNO"));
				lstM6Task.add(objDto);
			...
			if(lstM6Task.size()>0)
			{
			System.err.println("Service Disconnection Details Created And Stored In ArrayList");	
			iomsConn.setAutoCommit(false);
			csIOMS = iomsConn.prepareCall(strInsertECRMFXSERVICEDISCONNECT);
			...
			for (Iterator iter = lstM6Task.iterator(); iter.hasNext();) 
			{
				...
				csIOMS.setString(1, element.getServiceNo());
				csIOMS.setString(2, element.getOrderLineId());
				csIOMS.setString(3, element.getDisconnection_date());
				csIOMS.setString(4, element.getBillingTriggerDate());
				csIOMS.setInt(5, element.getFx_scheduler_status());
				csIOMS.setString(6, element.getCreatedDate());
				csIOMS.setString(7, element.getLastUpdatedDate1());
				csIOMS.setString(8, element.getTokenId());
				csIOMS.setString(9, element.getDisc_orderno());
				csIOMS.setString(10, element.getCrmOrderId());
				csIOMS.setInt(11, 0);
				csIOMS.setInt(12, 0);
				csIOMS.setString(13, "");
				csIOMS.execute();
			...
			if (csIOMS.getInt(12) == 0) {
				...
				//UpdateIOMSDataTracker("insert","TM_ACCOUNTROLEDETAILS",sdf.format(new Date()),sdf.format(new Date()));
				isInserted = true;
				System.err.println("Service Disconnection Details Updated....");
			} else {
				System.err.println("Service Disconnection Details Not Updated....");
				System.err.println(csIOMS.getString(8));
				iomsConn.rollback();
				isInserted = false;
			}
				...
				// Long ServiceSegment = null;
			System.out.println("Service Disconnection Details Inserted");
			} catch (Exception e) {
			System.out.println("Error in method NRC"
					+ e.getStackTrace());
			e.printStackTrace();
...
/*		System.err.println("Order Insertion....Started");
		...
		InsertECRMOrderLinetoIOMS();
		 		...
		 		System.err.println("Order Attributes Insertion....Started");
				InsertECRMHeaderAttributesInfotoIOMS2();  //NEED TO CHECK
				...
				System.err.println("Service Master Insertion....Started");
				InsertECRMServicetoIOMS2();
				
				System.err.println("TPOSERVICEDETAILS Insertion....Started");//TPOSERVICEDETAILS
				InsertECRMOrderLinetoIOMS2();*/
		 	System.err.println("FX Account Creation Details For New_Change_Order Insertion....Started");
			InsertECRMFXAccountCreationtoIOMS_Change();
			
			System.err.println("FX Service Creation Details For New_Change_Order Insertion....Started");
			 InsertECRMFXServiceCreationtoIOMS_Change();
			
			 System.err.println("FX RC Charge Creation Details For New_Change_Order Insertion....Started");
			 InsertECRMFXRCCreationtoIOMS_Change();
			
			 System.err.println("FX NRC Charge Creation Details For New_Change_Order Insertion....Started");
			 InsertECRMFXNRCCreationtoIOMS_Change();
			
			 System.err.println("FX Charge Disconnect Details For New_Change_Order Insertion....Started");
			 InsertECRMFXDISCONNECTIONCHARGE();
			
			 System.err.println("FX Service Update Details For New_Change_Order Insertion....Started");
			 InsertECRMFXSERVICEUPDATE();
			
			 System.err.println("FX Serice Disconnection Details For New_Change_Order Insertion....Started");
			 InsertECRMFXSERVICEDISCONNECT();
				...
				System.err.println("Order Line Insertion....Started");
				InsertECRMServiceAttributestoIOMS2();		
		...
		System.err.println("FX Auto Billing Details For New_Change_Order Insertion....Started");
		InsertECRMFXTAUTOBILLING();
